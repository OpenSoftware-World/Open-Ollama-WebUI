# Open-Ollama-WebUI
![License: GPL](https://img.shields.io/badge/License-GPL-blue.svg)
![Release Date](https://img.shields.io/badge/Release%20Date-2025--8--25-brightgreen)

---

**OSW-Ollama-WebUI** is a simple Ollama web interface built with Flask.
> 📅 Release date: **August 25, 2025**   
> 📦 Initial release: **Open Ollama WebUI 1.0**   
> 📦 Current version: **Open Ollama WebUI 1.1**   
> 📦 Next release: **Open Ollama WebUI 1.2**   
> ⏳ Next release status: **Under development...**   
> 🛠️ Next release type: **Big Update** 

---

## 🖥️ Features

- ✅ Open Ollama WebUI is a web interface that is quite simple to use.

---

---

## 🚀 How to Use?

1. Download and extract the OSW-Ollama-WebUI [from here](https://github.com/OpenSoftware-World/Open-Ollama-WebUI/releases).  
2. Move the `Open Ollama WebUI` folder to your project directory.  
3. Run the file named webui.py and enter the address specified in the command line into your browser.
